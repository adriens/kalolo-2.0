...
export class AuteurComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.auteurService
      ...
      .subscribe(
        (res: HttpResponse<IAuteur[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInAuteurs(): void {
    this.eventSubscriber = this.eventManager.subscribe('auteurListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAuteur[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
