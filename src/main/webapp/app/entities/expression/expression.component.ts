...
export class ExpressionComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.expressionService
      ...
      .subscribe(
        (res: HttpResponse<IExpression[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInExpressions(): void {
    this.eventSubscriber = this.eventManager.subscribe('expressionListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IExpression[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
