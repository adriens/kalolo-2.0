...
export class TypeMediaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.typeMediaService
      ...
      .subscribe(
        (res: HttpResponse<ITypeMedia[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTypeMedias(): void {
    this.eventSubscriber = this.eventManager.subscribe('typeMediaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITypeMedia[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-media'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
