...
/**
 * Performance test for the Auteur entity.
 */
class AuteurGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Auteur entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all auteurs")
            .get("/api/auteurs")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new auteur")
            .post("/api/auteurs")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nom":"SAMPLE_TEXT"
                , "cleAuteur":"SAMPLE_TEXT"
                , "urlWeb":"SAMPLE_TEXT"
                , "urlFb":"SAMPLE_TEXT"
                , "urlInsta":"SAMPLE_TEXT"
                , "urlYt":"SAMPLE_TEXT"
                , "urlTwit":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_auteur_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created auteur")
                .get("${new_auteur_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created auteur")
            .delete("${new_auteur_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
