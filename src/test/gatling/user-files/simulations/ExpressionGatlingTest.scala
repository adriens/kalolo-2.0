...
/**
 * Performance test for the Expression entity.
 */
class ExpressionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Expression entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all expressions")
            .get("/api/expressions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new expression")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_expression_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created expression")
                .get("${new_expression_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created expression")
            .delete("${new_expression_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
