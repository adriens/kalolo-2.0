...
/**
 * Performance test for the Media entity.
 */
class MediaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Media entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all media")
            .get("/api/media")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new media")
            .post("/api/media")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created media")
                .get("${new_media_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created media")
            .delete("${new_media_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
