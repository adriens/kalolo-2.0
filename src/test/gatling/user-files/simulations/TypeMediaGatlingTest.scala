...
/**
 * Performance test for the TypeMedia entity.
 */
class TypeMediaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TypeMedia entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all typeMedias")
            .get("/api/type-medias")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new typeMedia")
            .post("/api/type-medias")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "libelle":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_typeMedia_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created typeMedia")
                .get("${new_typeMedia_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created typeMedia")
            .delete("${new_typeMedia_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
