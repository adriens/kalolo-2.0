...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    expect(await tagComponentsPage.getTitle()).to.eq('kaloloApp.tag.home.title');
  ...
  it('should load create Tag page', async () => {
    ...
    expect(await tagUpdatePage.getPageTitle()).to.eq('kaloloApp.tag.home.createOrEditLabel');
  ...
  it('should create and save Tags', async () => {
    ...
    await promise.all([tagUpdatePage.setLibelleInput('libelle'), tagUpdatePage.setDefinitionInput('definition')]);

    expect(await tagUpdatePage.getLibelleInput()).to.eq('libelle', 'Expected Libelle value to be equals to libelle');
    expect(await tagUpdatePage.getDefinitionInput()).to.eq('definition', 'Expected Definition value to be equals to definition');
  ...
  it('should delete last Tag', async () => {
    ...
    expect(await tagDeleteDialog.getDialogTitle()).to.eq('kaloloApp.tag.delete.question');
